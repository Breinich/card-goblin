...
| ◆57 | Image masks (§3.3.5) | Image accepts `mask: none\|circle\|rounded`, quoted `icon:CODE` references, and optional `mask_radius:`; clipping is paint-only and shared by preview/PDF | Common portrait/icon-like crops need deterministic clipping without changing the existing box/layout model |
